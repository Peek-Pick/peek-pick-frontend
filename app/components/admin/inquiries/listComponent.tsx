...
function ListComponent({items, waitingAnswerOnly}: AdminInquiryListComponentProps) {
    const navigate = useNavigate();
    const location = useLocation(); // 현재 location 사용

    const handleRowClick = (id: number) => {
        // 현재 쿼리스트링을 유지하면서 상세 페이지로 이동
        ...
        <div>
            <div className="overflow-x-auto bg-white shadow rounded-lg">
                <table className="min-w-full text-sm divide-y divide-gray-200">
                    ...
                    <tr>
                        {['#', '유형', '작성일', '수정일', '닉네임', '상태', '삭제여부'].map((col) => (
